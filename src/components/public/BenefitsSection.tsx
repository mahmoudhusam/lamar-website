...
export default function BenefitsSection({ lang }: { lang: Lang }) {
  ...
  const alphas = [0.16, 0.12, 0.09, 0.07]
    ...
    <section id="benefits" style={{ background: 'var(--bg2)', padding: '7rem 3.5rem' }}>
      <div className="ben-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3.5rem', maxWidth: 1200, margin: '0 auto', alignItems: 'center' }}>
        ...
        <div className="rv">
          ...
          <div style={{ borderRadius: 26, padding: '1rem', background: 'linear-gradient(160deg, var(--teal2) 0%, var(--teal) 100%)', boxShadow: '0 24px 60px rgba(26,107,96,0.28)' }}>
            {tr.items.map((it, i) => (
              <div key={i} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', background: `rgba(255,255,255,${alphas[i] ?? 0.1})`, border: '1px solid rgba(255,255,255,0.18)', borderRadius: 18, padding: '1.1rem 1.25rem', marginTop: i === 0 ? 0 : '0.6rem' }}>
                <span style={{ width: 46, height: 46, borderRadius: '50%', border: '2px solid rgba(255,255,255,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><BIcon name={it.icon} /></span>
                <div>
