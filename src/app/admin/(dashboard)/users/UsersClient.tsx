...
type UserRow = { id: string; name: string; email: string; role: Role; createdAt: Date }

const ALL_ROLES: Role[] = ['SUPER_ADMIN', 'PROJECTS_EDITOR', 'CONTENT_EDITOR', 'GALLERY_EDITOR']
...
function EditUserForm({ user, currentUserId, onDone }: { user: UserRow; currentUserId: string; onDone: () => void }) {
  const [error, setError] = useState('')
  const [isPending, startTransition] = useTransition()
  ...
  function handleDelete(id: string) {
    ...
    startTransition(async () => {
      try {
        ...
        setDeleteError((prev) => ({ ...prev, [id]: err instanceof Error ? err.message : 'Delete failed.' }))
      }
    })
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {users.map((user) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
                ...
                <button
                  ...
                  style={{ ...btnDanger, opacity: user.id === currentUserId ? 0.3 : 1 }}
            ...
            {/* Delete error */}
            {deleteError[user.id] && (
              <p style={{ color: '#E05C5C', fontSize: '0.8rem', marginTop: '0.5rem' }}>{deleteError[user.id]}</p>
            )}

            {/* Edit form */}
            {editingId === user.id && (
              <EditUserForm user={user} currentUserId={currentUserId} onDone={() => setEditingId(null)} />
            )}
          </div>
        ))}
      </div>
